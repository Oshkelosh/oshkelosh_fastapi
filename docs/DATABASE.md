...
Oshkelosh supports two database backends, selected via `DATABASE_BACKEND` or the `DEPLOYMENT_PROFILE` preset in [`.env`](../.env.example).
...
**Images** live only in `product_images` (shared when `variant_id` is null, or tied to a variant). API `ProductRead.images` is assembled from those rows at read time — add images via admin upload or supplier sync.
